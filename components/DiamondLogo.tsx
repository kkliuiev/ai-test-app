...
interface Props {
  size?: 'sm' | 'md' | 'lg' | 'xl';
  showLabel?: boolean;
  /** Use white text (for dark backgrounds) — default true */
  light?: boolean;
}
...
export default function DiamondLogo({ size = 'md', showLabel = false, light = true }: Props) {
  const s = SIZES[size];
  const h = s.diamond;
  ...
  return (
    <View style={styles.wrapper}>
      ...
      >
        <Path d={DIAMOND_PATH} fill="#0057FF" fillRule="evenodd" clipRule="evenodd" />
      </Svg>

      {showLabel && (
        <View style={styles.labelCol}>
          <Text style={[styles.label, { fontSize: s.labelSize, color: light ? Colors.textPrimary : '#0B0F1C' }]}>
            AMLBot
          </Text>
          <Text style={[styles.sub, { fontSize: s.subSize }]}>Crypto Risk Analyzer</Text>
        </View>
      )}
    </View>
  );
