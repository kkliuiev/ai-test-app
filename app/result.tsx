...
export default function ResultScreen() {
  ...
  const { address, chain, riskLevel, riskScore, riskFactors, checkedAt, fromCache } = result;
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={[styles.scoreCard, { borderColor: riskBorder(riskLevel) }]}>
          ...
          <View style={styles.headerRight}>
            {fromCache && (
              <View style={styles.cachedPill}>
                <Text style={styles.cachedText}>{t('result.cached')}</Text>
              </View>
            )}
        ...
        <View style={styles.statsRow}>
          <Stat label={t('result.riskFactors')} value={String(riskyFactors.length)} color={color} />
          <View style={styles.statDiv} />
          <Stat label={t('result.cleanChecks')} value={String(safeFactors.length)} color={Colors.low} />
          <View style={styles.statDiv} />
          <Stat label={t('result.totalChecks')} value={String(riskFactors.length)} color={Colors.textSecondary} />
        </View>
      </View>

      <TouchableOpacity onPress={handleCopy} activeOpacity={0.75} style={styles.addressCard}>
        <Text style={styles.addressLabel}>{t('result.address')}</Text>
        <Text style={styles.addressText}>{address}</Text>
        <Text style={styles.copyHint}>{copied ? t('result.copied') : t('result.tapToCopy')}</Text>
      </TouchableOpacity>
      ...
      {riskScore === 0 ? (
        <View style={styles.cleanBanner}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.cleanTitle}>{t('result.noRisk')}</Text>
            <Text style={styles.cleanDesc}>{t('result.noRiskDesc')}</Text>
          </View>
        </View>
      ) : (
        <View style={[styles.riskBanner, { borderColor: riskBorder(riskLevel), backgroundColor: riskBg(riskLevel) }]}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={[styles.riskBannerTitle, { color }]}>{riskLevel} {t('result.riskDetected')}</Text>
            <Text style={styles.riskBannerDesc}>
              {riskyFactors.length === 1
                ? t('result.riskDesc', { n: riskyFactors.length })
                : t('result.riskDescPlural', { n: riskyFactors.length })}
            </Text>
          </View>
        </View>
      )}
      ...
      {riskyFactors.length > 0 && (
        <View style={styles.factorsSection}>
          <Text style={styles.sectionTitle}>{t('result.riskIndicators')}</Text>
      ...
      {safeFactors.length > 0 && (
        <View style={styles.factorsSection}>
          <Text style={styles.sectionTitle}>{t('result.cleanIndicators')}</Text>
      ...
      <View style={styles.actions}>
        ...
        <TouchableOpacity style={styles.newCheckBtn} onPress={() => router.push('/check')} activeOpacity={0.8}>
          <Text style={styles.newCheckBtnText}>{t('result.checkAnother')}</Text>
        </TouchableOpacity>
      </View>
    </ScrollView>
  );
...
function ErrorView() {
  ...
  return (
    <View style={styles.errContainer}>
      ...
      <Text style={styles.errText}>{t('result.notFound')}</Text>
      <TouchableOpacity style={styles.errBtn} onPress={() => router.back()}>
        <Text style={styles.errBtnText}>{t('result.goBack')}</Text>
      </TouchableOpacity>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  cachedText: { fontSize: 10, color: Colors.primaryLight, fontWeight: '700', letterSpacing: 0.5 },
  ...
  cleanBanner: {
    ...
    backgroundColor: Colors.lowBg, borderRadius: 14, padding: 16,
    borderWidth: 1, borderColor: Colors.lowBorder,
  },
  cleanIconWrap: {
    width: 40, height: 40, borderRadius: 20,
    backgroundColor: Colors.low, alignItems: 'center', justifyContent: 'center',
  },
  ...
  errIconWrap: {
    ...
    backgroundColor: Colors.criticalBg, alignItems: 'center', justifyContent: 'center',
  },
  errIcon: { fontSize: 28, color: Colors.critical },
  errText: { color: Colors.textPrimary, fontSize: 18, fontWeight: '700' },
  errBtn: { backgroundColor: Colors.primary, borderRadius: 28, paddingHorizontal: 32, paddingVertical: 14 },
